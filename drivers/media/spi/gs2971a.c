...
{
	struct gs2971a_priv *priv = container_of(sd, struct gs2971a_priv, subdev);
...
static const struct v4l2_subdev_pad_ops gs2971a_pad_ops = {
	...
	.get_mbus_config = gs2971a_g_mbus_config,
};
...
static int gs2971a_probe(struct spi_device *spi)
{
	...
	v4l2_spi_subdev_init(sd, spi, &gs2971a_subdev_ops);
	...
	snprintf(sd->name, sizeof(sd->name), "simple-sensor");
